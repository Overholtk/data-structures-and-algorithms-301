...
const createServer = () => {
  ...
  app.get('/aboutme', function(req,res){
    res.send('My name is Kjell and I like making art and coding');
  });
  app.get('/favoritefoods', function(req,res){
    res.send(['sushi', 'vegetable soup', 'blackberries']);
  });
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for(var i = 0; i < input.length; i++){
    let arr = input[i];
    let updated = arr.filter( (value, idx) => {
      return typeof value === 'number' && value % 5 === 0;
    });
    updated.forEach( (value,idx) => {
      updated[idx] = 2 ** value;
    });
    output.push(updated);
  }
