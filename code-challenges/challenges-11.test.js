...
const toLastNames = people => {
  let names = people.map( function(n,i) {
...
const validatePin = (pin) => {
  ...
  let check = pinString.match(validator);
  if(check){
    return true;
  }else{
    return false;
...
const validateEmail = (email) => {
  let validator = /^[A-Za-z0-9]+.?[A-Za-z0-9]+?@[A-Za-z0-9]+.(net|com|org)$/;
...
const validatePhoneNumber = (phoneNumber) => {
  ...
  let check = phoneNumber.match(validator);
  if(check){
    result = true;
  }
  return result;
